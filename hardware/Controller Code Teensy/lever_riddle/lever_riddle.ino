...
// Hall effect sensors (digital switch input)
const char *hall_sensors[] = {naming::SENSOR_MAGNET};
SentientDeviceDef dev_hall_a(naming::HALL_A, "Hall Sensor A", "hall_effect", naming::ACTION_TYPE_DIGITAL_SWITCH, hall_sensors, 1, true);
SentientDeviceDef dev_hall_b(naming::HALL_B, "Hall Sensor B", "hall_effect", naming::ACTION_TYPE_DIGITAL_SWITCH, hall_sensors, 1, true);
SentientDeviceDef dev_hall_c(naming::HALL_C, "Hall Sensor C", "hall_effect", naming::ACTION_TYPE_DIGITAL_SWITCH, hall_sensors, 1, true);
SentientDeviceDef dev_hall_d(naming::HALL_D, "Hall Sensor D", "hall_effect", naming::ACTION_TYPE_DIGITAL_SWITCH, hall_sensors, 1, true);

// Photocell (analog sensor input)
const char *photocell_sensors[] = {naming::SENSOR_LEVER_POSITION};
SentientDeviceDef dev_photocell(naming::PHOTOCELL, "Photocell Lever", "photocell", naming::ACTION_TYPE_ANALOG_SENSOR, photocell_sensors, 1, true);

// Cube button (digital switch input)
const char *button_sensors[] = {naming::SENSOR_BUTTON_STATE};
SentientDeviceDef dev_cube_button(naming::CUBE_BUTTON, "Cube Button", "button", naming::ACTION_TYPE_DIGITAL_SWITCH, button_sensors, 1, true);
...
SentientDeviceDef dev_ir_receiver(naming::IR_RECEIVER, "IR Receiver", "ir_receiver", naming::ACTION_TYPE_CODE_READER, ir_commands, 2, ir_sensors, 1);

// Maglock (digital relay output)
const char *maglock_commands[] = {naming::CMD_LOCK, naming::CMD_UNLOCK};
SentientDeviceDef dev_maglock(naming::MAGLOCK, "Maglock", "maglock", naming::ACTION_TYPE_RELAY, maglock_commands, 2);

// LED strips (RGB LED output)
const char *led_commands[] = {naming::CMD_SET_COLOR};
SentientDeviceDef dev_led_strip(naming::LED_STRIP, "Main LED Strip", "led_strip", naming::ACTION_TYPE_RGB, led_commands, 1);
SentientDeviceDef dev_led_lever(naming::LED_LEVER, "Lever LED Strip", "led_strip", naming::ACTION_TYPE_RGB, led_commands, 1);

// COB light (digital relay output)
const char *cob_commands[] = {naming::CMD_LIGHT_ON, naming::CMD_LIGHT_OFF};
SentientDeviceDef dev_cob_light(naming::COB_LIGHT, "COB Light", "light", naming::ACTION_TYPE_RELAY, cob_commands, 2);
...
void handle_mqtt_command(const char *command, const JsonDocument &payload, void *ctx)
{
    ...
    // Maglock commands
    if (strcmp(device_id, naming::MAGLOCK) == 0)
    {
    ...
    // LED strip commands
    else if (strcmp(device_id, naming::LED_STRIP) == 0 && strcmp(command, naming::CMD_SET_COLOR) == 0)
    {
        set_led_color(leds, NUM_LEDS, payload["color"]);
    }
    else if (strcmp(device_id, naming::LED_LEVER) == 0 && strcmp(command, naming::CMD_SET_COLOR) == 0)
    {
        set_led_color(lever_leds, NUM_LEVER_LEDS, payload["color"]);
    }
    // COB light commands
    else if (strcmp(device_id, naming::COB_LIGHT) == 0)
    {
    ...
    else if (strcmp(device_id, naming::IR_RECEIVER) == 0)
    {
...
void monitor_sensors()
{
    ...
    if (hall_a != last_hall_a)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::HALL_A, doc);
        last_hall_a = hall_a;
    }
    if (hall_b != last_hall_b)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::HALL_B, doc);
        last_hall_b = hall_b;
    }
    if (hall_c != last_hall_c)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::HALL_C, doc);
        last_hall_c = hall_c;
    }
    if (hall_d != last_hall_d)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::HALL_D, doc);
        last_hall_d = hall_d;
    }
    ...
    if (abs(photocell_value - last_photocell_value) > 50)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::PHOTOCELL, doc);
    ...
    if (cube_button != last_cube_button)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::CUBE_BUTTON, doc);
...
void handle_ir_signal()
{
    ...
    if (!is_duplicate && !is_weak && sentient.isConnected())
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::IR_RECEIVER, doc);
