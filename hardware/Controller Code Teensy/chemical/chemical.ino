...
void loop()
{
  ...
  // ────────────────────────────────────────────────────────────────────────────
  handle_rfid_reader(RFID_A, tag_buffer_a, &tag_present_a, &last_tir_a, tir_pin_a, naming::READER_A, naming::SENSOR_RFID_TAG_A);
  handle_rfid_reader(RFID_B, tag_buffer_b, &tag_present_b, &last_tir_b, tir_pin_b, naming::READER_B, naming::SENSOR_RFID_TAG_B);
  handle_rfid_reader(RFID_C, tag_buffer_c, &tag_present_c, &last_tir_c, tir_pin_c, naming::READER_C, naming::SENSOR_RFID_TAG_C);
  handle_rfid_reader(RFID_D, tag_buffer_d, &tag_present_d, &last_tir_d, tir_pin_d, naming::READER_D, naming::SENSOR_RFID_TAG_D);
  handle_rfid_reader(RFID_E, tag_buffer_e, &tag_present_e, &last_tir_e, tir_pin_e, naming::READER_E, naming::SENSOR_RFID_TAG_E);
  handle_rfid_reader(RFID_F, tag_buffer_f, &tag_present_f, &last_tir_f, tir_pin_f, naming::READER_F, naming::SENSOR_RFID_TAG_F);
...
void handle_mqtt_command(const char *command, const JsonDocument &payload, void * /*ctx*/)
{
  ...
  // Maglock Commands
  // ────────────────────────────────────────────────────────────────────────────
  else if (strcmp(device_id, naming::MAGLOCKS) == 0)
  {
