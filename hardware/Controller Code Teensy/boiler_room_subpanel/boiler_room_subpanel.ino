...
SentientDeviceDef dev_intro_tv_power(
    naming::INTRO_TV_POWER, naming::FRIENDLY_INTRO_TV_POWER, naming::TYPE_INTRO_TV_POWER,
    naming::ACTION_TYPE_RELAY,
    intro_tv_power_commands, 2);

SentientDeviceDef dev_intro_tv_lift(
    naming::INTRO_TV_LIFT, naming::FRIENDLY_INTRO_TV_LIFT, naming::TYPE_INTRO_TV_LIFT,
    naming::ACTION_TYPE_MOTOR,
    intro_tv_lift_commands, 3);

SentientDeviceDef dev_fog_power(
    naming::FOG_POWER, naming::FRIENDLY_FOG_POWER, naming::TYPE_FOG_POWER,
    naming::ACTION_TYPE_RELAY,
    fog_power_commands, 2);

SentientDeviceDef dev_fog_trigger(
    naming::FOG_TRIGGER, naming::FRIENDLY_FOG_TRIGGER, naming::TYPE_FOG_TRIGGER,
    naming::ACTION_TYPE_TRIGGER,
    fog_trigger_commands, 1);

SentientDeviceDef dev_fog_ultrasonic(
    naming::FOG_ULTRASONIC, naming::FRIENDLY_FOG_ULTRASONIC, naming::TYPE_FOG_ULTRASONIC,
    naming::ACTION_TYPE_RELAY,
    fog_ultrasonic_commands, 2);

SentientDeviceDef dev_barrel(
    naming::BOILER_ROOM_BARREL, naming::FRIENDLY_BOILER_ROOM_BARREL, naming::TYPE_BOILER_ROOM_BARREL,
    naming::ACTION_TYPE_RELAY,
    barrel_commands, 2);

SentientDeviceDef dev_ir_sensor(
    naming::IR_SENSOR, naming::FRIENDLY_IR_SENSOR, naming::TYPE_IR_SENSOR,
    naming::ACTION_TYPE_CODE_READER,
    ir_sensor_commands, 2,
    ir_sensor_sensors, 1);

SentientDeviceDef dev_study_door(
    naming::STUDY_DOOR, naming::FRIENDLY_STUDY_DOOR, naming::TYPE_STUDY_DOOR,
    naming::ACTION_TYPE_RELAY,
    study_door_commands, 2);

SentientDeviceDef dev_gauge_chest(
    naming::GAUGE_PROGRESS_CHEST, naming::FRIENDLY_GAUGE_PROGRESS_CHEST, naming::TYPE_GAUGE_PROGRESS_CHEST,
...
void setup()
{
  ...
  int seg = 0; const char *p = topic;
      ...
      // Dispatch
      // Intro TV Power
      if (device == naming::INTRO_TV_POWER) {
      ...
      // Intro TV Lift
      else if (device == naming::INTRO_TV_LIFT) {
      ...
      // Fog Machine Power
      else if (device == naming::FOG_POWER) {
      ...
      // Fog Machine Trigger
      else if (device == naming::FOG_TRIGGER) {
      ...
      // Fog Machine Ultrasonic
      else if (device == naming::FOG_ULTRASONIC) {
      ...
      // Study Door (group)
      else if (device == naming::STUDY_DOOR) {
      ...
      else if (device == naming::GAUGE_PROGRESS_CHEST) {
      ...
      // Barrel (maglock only)
      else if (device == naming::BOILER_ROOM_BARREL) {
      ...
      // IR Sensor (gun detection control)
      else if (device == naming::IR_SENSOR) {
...
void check_ir_sensor()
{
  ...
  if (!is_duplicate && !is_weak_signal)
  {
    ...
    String topic = String(mqtt_namespace) + "/" + room_id + "/" + naming::CAT_SENSORS + "/" + controller_id + "/" + naming::BOILER_ROOM_BARREL + "/ir_code";
    String payload;
