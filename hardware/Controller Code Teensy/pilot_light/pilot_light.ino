...
SentientDeviceDef dev_fire_leds(
    naming::FIRE_LEDS, naming::FRIENDLY_FIRE_LEDS, "led_strip", naming::ACTION_TYPE_RGB, fire_leds_commands, 2);

SentientDeviceDef dev_monitor_relay(
    naming::MONITOR_POWER_RELAY, naming::FRIENDLY_MONITOR_RELAY, "relay", naming::ACTION_TYPE_RELAY, monitor_commands, 2);

SentientDeviceDef dev_newell_relay(
    naming::NEWELL_POWER_RELAY, naming::FRIENDLY_NEWELL_RELAY, "relay", naming::ACTION_TYPE_RELAY, newell_commands, 2);

SentientDeviceDef dev_flange_leds(
    naming::FLANGE_LEDS, naming::FRIENDLY_FLANGE_LEDS, "led_strip", naming::ACTION_TYPE_RGB, flange_commands, 2);

SentientDeviceDef dev_color_sensor(
    naming::PILOTLIGHT_COLOR_SENSOR,
    naming::FRIENDLY_COLOR_SENSOR,
    "sensor",
    color_sensor_sensors, 1, true); // true = input device

SentientDeviceDef dev_controller(
    naming::CONTROLLER, naming::FRIENDLY_CONTROLLER, "controller", naming::ACTION_TYPE_RELAY, controller_commands, 2);
...
void handle_mqtt_command(const char *command, const JsonDocument &payload, void * /*ctx*/)
{
    ...
    const char *device_id = payload["device_id"].is<const char *>() ? payload["device_id"].as<const char *>() : naming::CONTROLLER_ID;
...
void check_and_publish_sensor_changes()
{
    ...
    if (changed || force_publish)
    {
        ...
        String item_temp = String(naming::PILOTLIGHT_COLOR_SENSOR) + "/" + naming::SENSOR_COLOR_TEMP;
