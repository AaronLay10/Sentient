...
SentientDeviceDef dev_motor_left(naming::MOTOR_LEFT, "Motor Left", "stepper_motor", naming::ACTION_TYPE_MOTOR, motor_commands, 3);
SentientDeviceDef dev_motor_right(naming::MOTOR_RIGHT, "Motor Right", "stepper_motor", naming::ACTION_TYPE_MOTOR, motor_commands, 3);
SentientDeviceDef dev_proximity_sensors(naming::PROXIMITY_SENSORS, "Proximity Sensors", "proximity_array", naming::ACTION_TYPE_DIGITAL_SWITCH, nullptr, 0, proximity_sensors, 8);
SentientDeviceDef dev_fog_dmx(naming::FOG_DMX, "DMX Fog Machine", "dmx_device", naming::ACTION_TYPE_PWM, fog_commands, 3);
...
void monitor_sensors()
{
        ...
        current.right_bottom_2 != sensor_last.right_bottom_2)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::PROXIMITY_SENSORS, doc);
        sensor_last = current;
    }
}
...
void handle_mqtt_command(const char *command, const JsonDocument &payload, void *ctx)
{
    ...
    // Motor Left
    if (strcmp(device_id, naming::MOTOR_LEFT) == 0)
    {
    ...
    // Motor Right
    if (strcmp(device_id, naming::MOTOR_RIGHT) == 0)
    {
        if (strcmp(command, naming::CMD_UP) == 0)
        {
            digitalWrite(PIN_MOTORS_POWER, HIGH);
    ...
    // DMX Fog Machine
    if (strcmp(device_id, naming::FOG_DMX) == 0)
    {
