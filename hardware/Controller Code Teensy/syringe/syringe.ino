...
SentientDeviceDef dev_encoder_lt(naming::ENCODER_LT, "Encoder Left Top", "rotary_encoder", encoder_sensors, 1, true);
SentientDeviceDef dev_encoder_lm(naming::ENCODER_LM, "Encoder Left Middle", "rotary_encoder", encoder_sensors, 1, true);
SentientDeviceDef dev_encoder_lb(naming::ENCODER_LB, "Encoder Left Bottom", "rotary_encoder", encoder_sensors, 1, true);
SentientDeviceDef dev_encoder_rt(naming::ENCODER_RT, "Encoder Right Top", "rotary_encoder", encoder_sensors, 1, true);
SentientDeviceDef dev_encoder_rm(naming::ENCODER_RM, "Encoder Right Middle", "rotary_encoder", encoder_sensors, 1, true);
SentientDeviceDef dev_encoder_rb(naming::ENCODER_RB, "Encoder Right Bottom", "rotary_encoder", encoder_sensors, 1, true);

// LED Ring devices (output)
const char *led_commands[] = {naming::CMD_SET_COLOR};
SentientDeviceDef dev_led_ring_a(naming::LED_RING_A, "LED Ring A", "led_ring", naming::ACTION_TYPE_RELAY, led_commands, 1);
SentientDeviceDef dev_led_ring_b(naming::LED_RING_B, "LED Ring B", "led_ring", naming::ACTION_TYPE_RELAY, led_commands, 1);
SentientDeviceDef dev_led_ring_c(naming::LED_RING_C, "LED Ring C", "led_ring", naming::ACTION_TYPE_RELAY, led_commands, 1);
SentientDeviceDef dev_led_ring_d(naming::LED_RING_D, "LED Ring D", "led_ring", naming::ACTION_TYPE_RELAY, led_commands, 1);
SentientDeviceDef dev_led_ring_e(naming::LED_RING_E, "LED Ring E", "led_ring", naming::ACTION_TYPE_RELAY, led_commands, 1);
SentientDeviceDef dev_led_ring_f(naming::LED_RING_F, "LED Ring F", "led_ring", naming::ACTION_TYPE_RELAY, led_commands, 1);

// Filament LED (output)
const char *filament_commands[] = {naming::CMD_LED_ON, naming::CMD_LED_OFF};
SentientDeviceDef dev_filament_led(naming::FILAMENT_LED, "Filament LED", "led", naming::ACTION_TYPE_RELAY, filament_commands, 2);
...
SentientDeviceDef dev_main_actuator(naming::MAIN_ACTUATOR, "Main Actuator", "actuator", naming::ACTION_TYPE_MOTOR, actuator_commands, 3);

// Forge Actuator (output)
const char *forge_commands[] = {naming::CMD_FORGE_EXTEND, naming::CMD_FORGE_RETRACT};
SentientDeviceDef dev_forge_actuator(naming::FORGE_ACTUATOR, "Forge Actuator", "actuator", naming::ACTION_TYPE_MOTOR, forge_commands, 2);
...
void handle_mqtt_command(const char *command, const JsonDocument &payload, void *ctx)
{
    ...
    // LED Ring commands
    if (strcmp(device_id, naming::LED_RING_A) == 0 && strcmp(command, naming::CMD_SET_COLOR) == 0)
    {
        set_led_ring_color(ring_a, payload["color"]);
        publish_command_acknowledgement(device_id, command);
    }
    else if (strcmp(device_id, naming::LED_RING_B) == 0 && strcmp(command, naming::CMD_SET_COLOR) == 0)
    {
        set_led_ring_color(ring_b, payload["color"]);
        publish_command_acknowledgement(device_id, command);
    }
    else if (strcmp(device_id, naming::LED_RING_C) == 0 && strcmp(command, naming::CMD_SET_COLOR) == 0)
    {
        set_led_ring_color(ring_c, payload["color"]);
        publish_command_acknowledgement(device_id, command);
    }
    else if (strcmp(device_id, naming::LED_RING_D) == 0 && strcmp(command, naming::CMD_SET_COLOR) == 0)
    {
        set_led_ring_color(ring_d, payload["color"]);
        publish_command_acknowledgement(device_id, command);
    }
    else if (strcmp(device_id, naming::LED_RING_E) == 0 && strcmp(command, naming::CMD_SET_COLOR) == 0)
    {
        set_led_ring_color(ring_e, payload["color"]);
        publish_command_acknowledgement(device_id, command);
    }
    else if (strcmp(device_id, naming::LED_RING_F) == 0 && strcmp(command, naming::CMD_SET_COLOR) == 0)
    {
        set_led_ring_color(ring_f, payload["color"]);
        publish_command_acknowledgement(device_id, command);
    }
    // Filament LED commands
    else if (strcmp(device_id, naming::FILAMENT_LED) == 0)
    {
    ...
    else if (strcmp(device_id, naming::MAIN_ACTUATOR) == 0)
    {
    ...
    else if (strcmp(device_id, naming::FORGE_ACTUATOR) == 0)
    {
