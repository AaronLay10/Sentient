...
SentientDeviceDef dev_hoist(naming::HOIST, "Hoist", "stepper_hoist", naming::ACTION_TYPE_MOTOR, hoist_commands, 3, hoist_sensors, 4);
SentientDeviceDef dev_left_door(naming::LAB_DOOR_LEFT, "Left Lab Door", "stepper_door", naming::ACTION_TYPE_MOTOR, door_commands, 3, door_sensors, 4);
SentientDeviceDef dev_right_door(naming::LAB_DOOR_RIGHT, "Right Lab Door", "stepper_door", naming::ACTION_TYPE_MOTOR, door_commands, 3, door_sensors, 4);
SentientDeviceDef dev_rope_drop(naming::ROPE_DROP, "Rope Drop", "solenoid", naming::ACTION_TYPE_RELAY, rope_commands, 2);
SentientDeviceDef dev_ir_receiver(naming::IR_RECEIVER, "Gun IR Receiver", "ir_receiver", naming::ACTION_TYPE_CODE_READER, nullptr, 0, ir_sensors, 1);
...
void monitor_sensors()
{
    ...
    {
        publish_sensor(naming::HOIST, naming::SENSOR_UP_A, hoist_up_a);
        hoist_up_a_last = hoist_up_a;
    }
    if (hoist_up_b != hoist_up_b_last)
    {
        publish_sensor(naming::HOIST, naming::SENSOR_UP_B, hoist_up_b);
        hoist_up_b_last = hoist_up_b;
    }
    if (hoist_down_a != hoist_down_a_last)
    {
        publish_sensor(naming::HOIST, naming::SENSOR_DOWN_A, hoist_down_a);
        hoist_down_a_last = hoist_down_a;
    }
    if (hoist_down_b != hoist_down_b_last)
    {
        publish_sensor(naming::HOIST, naming::SENSOR_DOWN_B, hoist_down_b);
    ...
    {
        publish_sensor(naming::LAB_DOOR_LEFT, naming::SENSOR_OPEN_A, left_open_a);
        left_open_a_last = left_open_a;
    }
    if (left_open_b != left_open_b_last)
    {
        publish_sensor(naming::LAB_DOOR_LEFT, naming::SENSOR_OPEN_B, left_open_b);
        left_open_b_last = left_open_b;
    }
    if (left_closed_a != left_closed_a_last)
    {
        publish_sensor(naming::LAB_DOOR_LEFT, naming::SENSOR_CLOSED_A, left_closed_a);
        left_closed_a_last = left_closed_a;
    }
    if (left_closed_b != left_closed_b_last)
    {
        publish_sensor(naming::LAB_DOOR_LEFT, naming::SENSOR_CLOSED_B, left_closed_b);
    ...
    {
        publish_sensor(naming::LAB_DOOR_RIGHT, naming::SENSOR_OPEN_A, right_open_a);
        right_open_a_last = right_open_a;
    }
    if (right_open_b != right_open_b_last)
    {
        publish_sensor(naming::LAB_DOOR_RIGHT, naming::SENSOR_OPEN_B, right_open_b);
        right_open_b_last = right_open_b;
    }
    if (right_closed_a != right_closed_a_last)
    {
        publish_sensor(naming::LAB_DOOR_RIGHT, naming::SENSOR_CLOSED_A, right_closed_a);
        right_closed_a_last = right_closed_a;
    }
    if (right_closed_b != right_closed_b_last)
    {
        publish_sensor(naming::LAB_DOOR_RIGHT, naming::SENSOR_CLOSED_B, right_closed_b);
...
void handle_mqtt_command(const char *command, const JsonDocument &payload, void *ctx)
{
    ...
    // Hoist
    if (strcmp(device_id, naming::HOIST) == 0)
    {
    ...
    // Left door
    if (strcmp(device_id, naming::LAB_DOOR_LEFT) == 0)
    {
    ...
    // Right door
    if (strcmp(device_id, naming::LAB_DOOR_RIGHT) == 0)
    {
        if (strcmp(command, naming::CMD_DOOR_OPEN) == 0)
        {
    ...
    // Rope drop
    if (strcmp(device_id, naming::ROPE_DROP) == 0)
    {
