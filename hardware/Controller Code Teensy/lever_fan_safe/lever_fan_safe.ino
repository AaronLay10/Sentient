...
SentientDeviceDef dev_photocell_safe(naming::PHOTOCELL_SAFE, "Safe Photocell", "photocell", naming::ACTION_TYPE_ANALOG_SENSOR, photocell_sensors, 1, true);
SentientDeviceDef dev_photocell_fan(naming::PHOTOCELL_FAN, "Fan Photocell", "photocell", naming::ACTION_TYPE_ANALOG_SENSOR, photocell_sensors, 1, true);

// IR receivers
const char *ir_sensors[] = {naming::SENSOR_IR_CODE};
const char *ir_commands[] = {naming::CMD_IR_ENABLE, naming::CMD_IR_DISABLE};
SentientDeviceDef dev_ir_safe(naming::IR_SAFE, "Safe IR Receiver", "ir_receiver", naming::ACTION_TYPE_CODE_READER, ir_commands, 2, ir_sensors, 1);
SentientDeviceDef dev_ir_fan(naming::IR_FAN, "Fan IR Receiver", "ir_receiver", naming::ACTION_TYPE_CODE_READER, ir_commands, 2, ir_sensors, 1);

// Maglock
const char *maglock_commands[] = {naming::CMD_LOCK, naming::CMD_UNLOCK};
SentientDeviceDef dev_maglock_fan(naming::MAGLOCK_FAN, "Fan Maglock", "maglock", naming::ACTION_TYPE_RELAY, maglock_commands, 2);

// Solenoid
const char *solenoid_commands[] = {naming::CMD_OPEN, naming::CMD_CLOSE};
SentientDeviceDef dev_solenoid_safe(naming::SOLENOID_SAFE, "Safe Solenoid", "solenoid", naming::ACTION_TYPE_RELAY, solenoid_commands, 2);

// Fan motor
const char *fan_commands[] = {naming::CMD_FAN_ON, naming::CMD_FAN_OFF};
SentientDeviceDef dev_fan_motor(naming::FAN_MOTOR, "Fan Motor", "motor", naming::ACTION_TYPE_MOTOR, fan_commands, 2);
...
void handle_mqtt_command(const char *command, const JsonDocument &payload, void *ctx)
{
    ...
    if (strcmp(device_id, naming::MAGLOCK_FAN) == 0)
    {
    ...
    else if (strcmp(device_id, naming::SOLENOID_SAFE) == 0)
    {
    ...
    // Fan motor commands
    else if (strcmp(device_id, naming::FAN_MOTOR) == 0)
    {
    ...
    else if ((strcmp(device_id, naming::IR_SAFE) == 0 || strcmp(device_id, naming::IR_FAN) == 0))
    {
...
void monitor_sensors()
{
    ...
    if (current_state_safe != last_state_safe)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::PHOTOCELL_SAFE, doc);
    ...
    if (current_state_fan != last_state_fan)
    {
        ...
        sentient.publishJson(naming::CAT_SENSORS, naming::PHOTOCELL_FAN, doc);
...
void handle_ir_signal(int pin)
{
    ...
    const char *device_id = (pin == PIN_IR_SAFE) ? naming::IR_SAFE : naming::IR_FAN;
